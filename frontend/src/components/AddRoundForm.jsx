...
import { Plus, Minus, Flag, AlertCircle } from "lucide-react";
...
function Stepper({ value, onChange }) {
  return (
    <div className="flex items-center gap-1 rounded-lg border border-neutral-border-default bg-neutral-surface-default p-1">
      <button
        type="button"
        onClick={() => onChange(Math.max(0, value - 1))}
        disabled={value <= 0}
        className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-text-subtle transition-colors hover:bg-neutral-surface-tinted disabled:opacity-30"
        aria-label="Decrease"
      >
        <Minus size={14} />
      </button>
      <span className="w-6 text-center text-sm font-semibold tabular-nums text-neutral-text-default">
        {value}
      </span>
      <button
        type="button"
        onClick={() => onChange(value + 1)}
        className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-text-subtle transition-colors hover:bg-neutral-surface-tinted"
        aria-label="Increase"
      >
        <Plus size={14} />
      </button>
    ...
    <Card>
      <CardContent className="pt-6">
        <div className="mb-4 flex items-center gap-2">
          <Flag size={18} className="text-neutral-text-default" />
          <h3 className="text-sm font-semibold text-neutral-text-default">Add round</h3>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col divide-y divide-neutral-border-subtle border border-neutral-border-subtle">
            {participants.map((p) => (
