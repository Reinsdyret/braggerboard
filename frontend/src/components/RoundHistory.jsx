        ...
        >
          <div className="mb-2.5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <strong className="text-sm font-semibold text-gray-900">{round.label || "Round"}</strong>
            </div>
            <span className="shrink-0 text-xs text-gray-400">{formatDate(round.createdAt)}</span>
          </div>
          <ul className="flex flex-wrap gap-1.5">
            {round.results.map((result) => (
              ...
              >
                {result.participantName} <span className="font-bold text-gray-800">×{result.wins}</span>
              </li>
            ))}
          </ul>
        </li>
