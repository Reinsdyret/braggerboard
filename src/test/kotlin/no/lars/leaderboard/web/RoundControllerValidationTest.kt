...
class RoundControllerValidationTest {
    ...
    fun `rejects a round result for a participant from a different leaderboard`() {
        val boardA = leaderboardRepository.create("Board A", ScoringMode.WIN_COUNT, null)
        val boardB = leaderboardRepository.create("Board B", ScoringMode.WIN_COUNT, null)
    ...
    fun `accepts a round result for a participant that does belong to the leaderboard`() {
        val board = leaderboardRepository.create("Board C", ScoringMode.WIN_COUNT, null)
