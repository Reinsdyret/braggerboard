...
class ParticipantControllerUpdateTest {
    ...
    fun `renaming a participant logs the old and new name`() {
        val board = leaderboardRepository.create("Board", ScoringMode.WIN_COUNT, "test-hash")
    ...
    fun `renaming to the exact same name does not create a log entry`() {
        val board = leaderboardRepository.create("Board 2", ScoringMode.WIN_COUNT, "test-hash")
    ...
    fun `adding then removing a photo logs both changes`() {
        val board = leaderboardRepository.create("Board 3", ScoringMode.WIN_COUNT, "test-hash")
    ...
    fun `rejects a spoofed non-image file on update, same as on create`() {
        val board = leaderboardRepository.create("Board 4", ScoringMode.WIN_COUNT, "test-hash")
    ...
    fun `rejects blank or too-long names on update`() {
        val board = leaderboardRepository.create("Board 5", ScoringMode.WIN_COUNT, "test-hash")
